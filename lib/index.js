...
JunitFullReporter = function (newman, reporterOptions) {
	...
	newman.on('beforeDone', function () {
        ...
        // _.forEach(executions, aggregator);
        ...
        // }
		let executionKeys = executions.map((execution) => execution.cursor.key)
		console.log(executionKeys);
		for (let key of executionKeys) {
			let executionsForKey = executions.filter((execution) => execution.cursor.key == key);
			// var testsuite = global.ele('testsuite'), failures = 0, errors = 0, propertyValues;
			console.log(executionsForKey[0])
			console.log(executionsForKey[0].cursor)

			// testsuite.att('id', (executionsForKey.cursor.iteration * executionsForKey.cursor.length) + executionsForKey.cursor.position);
		}

		_.forEach(executions, function (execution) {
			var testsuite = global.ele('testsuite');
			var failures = 0, errors = 0;
			// console.log(execution.cursor.ref)
			/**
			 * NS CHANGE 
			 * This is to ensure the env variables / global values do not come in XML report
			...
			/**
			 * **NS Changes
			 * Code changed to remove 'localhost' as last param to below .get
			 */
			var hostName = _.get(execution, 'request.url.host');
			...
			if (propertyValues && propertyValues.length) {
				...
				_.forEach(propertyValues, function (propertyItem) {
                    ...
                    /**
                    * NS Change: Change property.name !== to != 
                    */
					property.name != null ? property.att('name', propertyItem.key): property.att('name', "")
					property.value != null ? property.att('value', propertyItem.value): property.att('value', "")
				});
			}
            ...
            /**
             * NS Change:
             * Replace below forEach
             * Actual: _.forEach(['prerequestScript', 'assertions', 'testScript'], function (property) {
             */
