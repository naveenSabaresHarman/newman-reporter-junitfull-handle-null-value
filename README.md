...
```console
$ npm install -g newman-reporter-junitnull
```
...
JUnit examples can be found [here](https://github.com/martijnvandervlag/newman-reporter-junitnull/tree/master/examples).
...
In order to enable this reporter, specify `junitnull` in Newman's `-r` or `--reporters` option.

```console
newman run https://www.getpostman.com/collections/631643-f695cab7-6878-eb55-7943-ad88e1ccfd65-JsLv -r junitnull --reporter-junitnull-export './examples/full/result.xml' -n 2
```
...
|-------------|-------------------|
| `--reporter-junitnull-export <path>` | Specify a path where the output XML file will be written to disk. If not specified, the file will be written to `newman/` in the current working directory. |
...
newman.run({
    ...
    reporter: {
        junitnull: {
            export: './examples/null/result.xml', // If not specified, the file will be written to `newman/` in the current working directory.
        }
    },
	iterationCount: 2
}, function (err) {
...
| **newman-reporter-junitnull** | **newman** | **node** |
|:-----------------------------:|:----------:|:--------:|
|            v1.0.0             | >= v4.0.0  | >= v6.x  |
|            v1.1.0             | >= v4.0.0  | >= v6.x  |
...
> If you are facing any other problem, please check the open [issues](https://github.com/martijnvandervlag/newman-reporter-junitnull/issues) or create new.
